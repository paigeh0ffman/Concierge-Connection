...
MigraineInsights analyzeMigraineLogs(
  ...
  if (highPainDays >= 3) {
    flags.add(MigraineFlag(
      ...
      detail:
          'Pain rated 7+ for $highPainDays consecutive days. Appointment recommended.',
    ));
  }
  ...
  if (painTrend >= 3) {
    flags.add(MigraineFlag(
      ...
      detail:
          'Pain rose from ${safeInt(first, 'pain')} to ${safeInt(last, 'pain')} over ${recent.length} days.',
    ));
  }

  // Yellow — aura in 3+ of last 4 logs
  ...
  if (recentAura >= 3) {
    flags.add(MigraineFlag(
      ...
      label: 'Frequent aura',
      detail:
          'Aura reported in $recentAura of the last ${recent.length} logs. Worth discussing.',
    ));
  }

  // Yellow — high nausea for 3+ days
  ...
  if (highNauseaDays >= 3) {
    flags.add(MigraineFlag(
      ...
      detail:
          'High nausea reported for $highNauseaDays of the last ${recent.length} days.',
    ));
  }

  // Yellow — long duration migraines
  ...
  if (longMigraines >= 2) {
    flags.add(MigraineFlag(
      ...
      detail:
          '$longMigraines migraines lasting 24+ hours in the last ${recent.length} logs.',
    ));
  }
